<template>
  <aside class="drawer-side bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700">
    <label for="my-drawer" aria-label="Close sidebar" class="drawer-overlay"></label>

    <nav class="menu p-4 w-80 min-h-full text-gray-800 dark:text-gray-200" aria-label="Main navigation">
      <!-- Header -->
      <header class="mb-6 px-4 py-3 flex items-center border-b border-gray-200 dark:border-gray-700">
        <h1 class="text-lg font-bold text-gray-900 dark:text-white">Eggflow</h1>
      </header>

            <ul class="space-y-1" role="list">
        <li v-for="(link, index) in links" :key="index" class="relative">
          <button
            class="flex items-center w-full px-4 py-3 rounded-lg text-left transition-all duration-200 hover:bg-gray-100 dark:hover:bg-gray-700"
            :class="[
              link.active
          ...
          >
            <ul
              ...
              class="mt-1 ml-9 space-y-1 border-l-2 border-gray-200 dark:border-gray-700 pl-4"
              role="group"
            >
              <li v-for="(child, childIndex) in link.children" :key="childIndex">
                <a
                  href="#"
                  class="block px-3 py-2 rounded-md text-sm transition-colors duration-200 hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300"
